...
module CONTROLLER(
    ...
    input wire zero_i, //ALU零标志位输入
    input wire neg_i, //ALU负标志位输入
    output reg [1:0] npc_op_o, 
    output reg rf_we_o, //寄存器写使能
    output reg [1:0] rf_wd_sel_o, //寄存器写数据选择
    output reg [3:0] ext_op_o, //立即数扩展操�?
    output reg [3:0] alu_op_o, //ALU操作�?
    output reg [1:0] alu_b_sel_o, //ALU操作数B
    output reg [1:0] ram_op_o, //RAM操作
    output reg [1:0] mux_sel_o, //PC多路选择器�?�择
    output wire br_o
);
...
assign br_o = (opcode == `OPCODE_BRANCH) ? 
              ((fun3 == `FUN3_BEQ) ? (zero_i) :
               (fun3 == `FUN3_BNE) ? (~zero_i) :  // BNE分支判断，zero_i取反
               (fun3 == `FUN3_BLT) ? (neg_i) : 
                (fun3 == `FUN3_BGE) ? (~neg_i) :  // BGE分支判断，neg_i取反
                1'b0) : // 其他情况非分支指令
              1'b0;
